...
// each test a process and based on the _exit status of the process,
...
// try to find any races between _exit and wait
void
_exitwait(char *s)
{
  int i, pid;
  ...
  for(i = 0; i < 100; i++){
    ...
    if(pid){
      ...
      if(i != xstate) {
        printf("%s: wait wrong _exit status\n", s);
        _exit(1);
      }
    } else {
      _exit(i);
...
// code that handles a parent _exiting
