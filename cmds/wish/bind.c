  ...
  char *argv[];
{
  ...
  switch (argc)
  {
    case 4:
      key = (uchar *)argv[2];
      cmd = (uchar *)argv[3];		/* Bind a string to another */
      goto bindit;		/* Yuk, a goto */
    case 3:
      key = (uchar *)argv[1];
      cmd = (uchar *)argv[2];		/* Bind a string to another */
  bindit:
...
void initbind()
 {
  ...
  for (i = 0; i < 15; i++)
    Bind(3, (char **) defbind[i]);	/* Set default bindings */
